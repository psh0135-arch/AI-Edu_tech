...
const tools = [
  ...
  { name: 'Gamma AI', category: '프레젠테이션', emoji: '📊', practice: true, color: 'from-violet-600/20 to-violet-900/20', border: 'border-violet-500/30', text: 'text-violet-400' },
  ...
  { name: 'NotebookLM', category: 'AI 리서치', emoji: '📓', practice: false, color: 'from-cyan-600/20 to-cyan-900/20', border: 'border-cyan-500/30', text: 'text-cyan-400' },
  ...
  { name: 'Cursor AI', category: '바이브코딩', emoji: '💻', practice: true, color: 'from-slate-600/20 to-slate-900/20', border: 'border-slate-500/30', text: 'text-slate-300' },
]
  ...
  return (
    <section id="tools" className="py-24 relative overflow-hidden" ref={ref}>
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-0 w-80 h-80 bg-cyan-600/10 rounded-full blur-[120px]" />
      </div>
        ...
        <div className="mt-16 overflow-hidden">
          <div className="flex animate-marquee gap-6 w-max">
            {[...tools, ...tools].map((tool, i) => (
              <div
                key={i}
                className="flex items-center gap-2 px-4 py-2 glass border border-purple-500/20 rounded-full text-sm text-slate-400 flex-shrink-0"
              >
