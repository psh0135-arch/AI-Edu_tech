    ...
    <div className="min-h-screen bg-[#020617] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          ...
          <span className="text-white font-bold text-xl">AI마케팅 <span className="text-slate-400 text-sm font-normal">관리자</span></span>
        </div>
        ...
        <div className="glass rounded-2xl border border-purple-500/20 p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-semibold rounded-xl hover:from-purple-500 hover:to-cyan-500 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? '로그인 중...' : '로그인'}
            </button>
          </form>
...
function DetailModal({ item, onClose }: { item: Enrollment; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg bg-[#0f172a] border border-purple-500/20 rounded-2xl overflow-hidden max-h-[90vh] flex flex-col">
        {/* Header */}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="rounded-xl bg-white/3 border border-white/5 p-4 space-y-2 text-sm">
            ...
            {item.goal && (
              <div className="pt-2 border-t border-white/5">
                ...
                <p className="text-slate-300 text-sm mt-0.5 leading-relaxed">{item.goal}</p>
              </div>
            )}
          </div>
          ...
          {/* 상담 상태 */}
          <div>
            <label className="block text-slate-400 text-xs font-medium mb-1.5">상담 상태</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-slate-400 text-xs font-medium mb-1.5">결제 상태</label>
              <div className="relative">
                <select
                  value={paymentStatus}
                  onChange={e => setPaymentStatus(e.target.value as PaymentStatus)}
                  className="w-full bg-slate-800/60 border border-slate-700/60 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-purple-500/60 transition-all appearance-none pr-8"
                  style={{ colorScheme: 'dark' }}
                >
            ...
            <div>
              <label className="block text-slate-400 text-xs font-medium mb-1.5">결제 금액 (원)</label>
              <input
                type="number"
                value={paymentAmount}
          ...
          {/* 상담 메모 */}
          <div>
            <label className="block text-slate-400 text-xs font-medium mb-1.5">상담 메모</label>
            <textarea
              value={memo}
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-semibold rounded-xl hover:from-purple-500 hover:to-cyan-500 transition-all disabled:opacity-60"
          >
            {saving ? (
...
function Dashboard({ user }: { user: User }) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white">
      {/* Navbar */}
      <div className="glass border-b border-purple-500/10 sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
          {[
            ...
            { icon: MessageSquare, label: '상담 진행중', value: `${kpi.consulting}명`, color: 'text-cyan-400' },
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-4 py-2 bg-slate-800/60 border border-slate-700/60 text-white placeholder-slate-500 rounded-xl text-xs focus:outline-none focus:border-purple-500/60 transition-all w-52"
            />
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-slate-800/60 border border-slate-700/60 text-slate-300 hover:text-white hover:border-slate-500 rounded-xl transition-all"
          >
                ...
                <tbody>
                  {filtered.map(e => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-400 whitespace-nowrap">{e.phone}</td>
                      <td className="px-4 py-3 text-slate-400 max-w-[180px] truncate">{e.email}</td>
                      <td className="px-4 py-3 text-slate-400 whitespace-nowrap">{e.affiliation || '-'}</td>
                      <td className="px-4 py-3 text-slate-400 whitespace-nowrap text-xs">{e.aiLevel.split(' ')[0]}</td>
